...
public class IdentityProcessor {
    ...
    public static Map<String, String> accountFromUsername(String username) {
        ...
        if (local) {
            String email = getSingleValuedAttr(user, EMAIL);
            String inum = getSingleValuedAttr(user, INUM_ATTR);
            String name = getSingleValuedAttr(user, GIVEN_NAME);
    ...
    public static Map<String, String> remoteAccountDetails(String id) {

        String extUid = externalIdOf(id);
    ...
    public static String link(String encInum, String id) throws Exception {
        ...
        for (String uid : extUids) {
            if (!uid.startsWith(EXT_UID_PREFIX)) {
                uids.add(uid);
            }
        }

        user.setAttribute(EXT_ATTR, uids.toArray(new String[0]), true);
        // The setAttribute(String, Set<String>, boolean) version does weird stuff when
        // used from Groovy
